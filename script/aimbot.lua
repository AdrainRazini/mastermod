...
function createUI()
	...
	statusDot.BackgroundColor3 = Color3.fromRGB(255, 0, 0) -- vermelho inicial
	...
	function updateStatusDotColor(target)
		if not aimEnabled then
			statusDot.BackgroundColor3 = Color3.fromRGB(255, 255, 0) -- Amarelo (Desativado)
		elseif target then
			statusDot.BackgroundColor3 = Color3.fromRGB(0, 255, 0) -- Verde (Mirando)
		else
			statusDot.BackgroundColor3 = Color3.fromRGB(255, 0, 0) -- Vermelho (Sem alvo)
		end
	end
	...
	-- Botões para alternar configurações
	createButton("Desativar Mira", function(btn)
		aimEnabled = not aimEnabled
		btn.Text = aimEnabled and "Desativar Mira" or "Ativar Mira"
	end)
	...
	createButton("Mirar em Todos", function(btn)
		targetEnemiesOnly = not targetEnemiesOnly
		btn.Text = targetEnemiesOnly and "Mirar na Equipe Inimiga" or "Mirar em Todos"
	end)

	createButton("Mirar em NPCs: Sim", function(btn)
		includeNPCs = not includeNPCs
		btn.Text = "Mirar em NPCs: " .. (includeNPCs and "Sim" or "Não")
	end)
end
...
function findClosestPlayer()
	...
	-- Procurar NPCs no workspace e dentro de pastas
	if includeNPCs then
		for _, npc in ipairs(workspace:GetDescendants()) do
			if npc:IsA("Model") and not players:GetPlayerFromCharacter(npc) then
				local hum = npc:FindFirstChildOfClass("Humanoid")
				local head = npc:FindFirstChild("Head")
				if hum and head and hum.Health > 0 then  -- Verificação de saúde maior que 0
					local distance = (hrp.Position - head.Position).Magnitude
					if distance < closestDistance then
						closestDistance = distance
...
-- Criar UI e atualizar se o personagem morrer
